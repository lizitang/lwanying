        ...
        $.ajax({
            ...
            success:function(data){
                ...
                for(var i = 0;i<data.result.length;i++){
                    var content = answerFromCustomer(data.result[i].user_question.content, userName);
                    appendContent(content);
                    var content = answerFromBot(data.result[i].robot_answer.content, BOT_NAME);
                    appendContent(content);
                    //生成第二列
                    appendlist2()
                    // //获取第二列定位高度
...
//end
    ...
    var waitBotResponse = function (/*String*/userMsg){
        ...
        var requestURL = 'http://192.168.2.197:8081/cu?UniqueID=123&UserID='+sessionID+'&robot=proactive&Text1=' + encodeURIComponent(userMsg)+Math.random();
        var url ="http://localhost:8089/proxy?target=" + encodeURIComponent(requestURL);
        // ajax get response
        $.ajax({
            url: url,
            type: 'GET',
                ...
                if(memory !== void 0){
                    $.each(memory, function(i, memoryFeature){
                        ...
                        if(memoryFeature.type == 'proactive'){
                            // get answer
                            answer = memoryFeature.candidateAnswer;
                            // split answer
        ...
        //得到bot的act
        var requestURL5 ='http://192.168.2.211:9500/nlu?text='+ encodeURIComponent(answer);
        var url5 ="http://localhost:8089/proxy?target=" + encodeURIComponent(requestURL5);
